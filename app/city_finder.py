...
Fix: estado persistente entre pasos + fallback estimated_salary.
"""
...
COUNTRY_LANGUAGES = {
    ...
    "Czech Republic": "Checo", "Hungary": "Húngaro",
    "Romania": "Rumano", "Bulgaria": "Búlgaro",
    "Croatia": "Croata", "Slovakia": "Eslovaco",
    "Slovenia": "Esloveno", "Estonia": "Estonio",
    "Latvia": "Letón", "Lithuania": "Lituano",
    "Greece": "Griego", "Luxembourg": "Francés/Alemán",
    "Turkey": "Turco", "South Africa": "Inglés",
}
...
def _get_estimated_salary(df: pd.DataFrame) -> float:
    """
    Obtiene el salario estimado del perfil.
    Fallback: mediana del dataset si el modelo no pudo predecir.
    """
    ...
    # Fallback — mediana global del dataset de salarios
    try:
        sal_path = PROCESSED / "salaries.csv"
        if sal_path.exists():
            df_sal    = pd.read_csv(sal_path)
            origin    = profile.get("origin_country", "")
            role      = profile.get("role", "")
            # Intentar mediana por país y rol
            subset = df_sal[
                (df_sal["city_name"] == origin) &
                (df_sal["role"] == role)
            ]
            if len(subset) > 0:
                return float(subset["gross_salary"].median())
            # Fallback por país
            subset = df_sal[df_sal["city_name"] == origin]
            if len(subset) > 0:
                return float(subset["gross_salary"].median())
        # Fallback global
        return float(df["average_salary"].median())
    except Exception:
        return 55000.0
...
def step_donde():
    ...
    # Recuperar valores previos para mantener estado
    ...
    with col1:
        regiones = st.multiselect(
            ...
            default=finder.get("regiones", ["Europe"]),
        )
        clima = st.select_slider(
            "¿Qué clima prefieres?",
            options=["frío", "templado", "cálido", "me da igual"],
            value=finder.get("clima", "templado"),
        )
    with col2:
        idiomas = st.multiselect(
            ...
            default=finder.get("idiomas", ["Inglés", "Español"]),
        )
        solo_idioma = st.toggle(
            "Prefiero un país donde pueda comunicarme",
            value=finder.get("solo_idioma", True),
        )
...
def _build_radar(values: dict) -> go.Figure:
    ...
    fig.update_layout(
        polar=dict(
            ...
            angularaxis=dict(
                gridcolor="#2c2c2e", linecolor="#3a3a3c",
...
def step_prioridades():
    ...
    estimated = p.get("estimated_salary") or 0
    finder    = st.session_state.get("finder", {})

    _section_title(
        "¿Qué valoras más en tu nueva vida?",
        f"Mueve los sliders para definir tus prioridades. "
        f"El radar se adapta automáticamente. "
        f"{'Salario estimado: €' + f'{estimated:,.0f}/año' if estimated else 'Salario calculado desde el dataset.'}"
    )

    # Recuperar prioridades previas si existen
    prev_pri = finder.get("prioridades", {})
    defaults = {"ahorro": 50, "wlb": 40, "tech": 35, "cultura": 30, "estabilidad": 45}

    for k, v in defaults.items():
        key = f"pri_{k}"
        if key not in st.session_state:
            # Intentar recuperar del estado previo del finder
            if prev_pri:
                total_prev = sum(prev_pri.values()) or 1
                st.session_state[key] = int(prev_pri.get(k, v) / 100 * total_prev) if prev_pri else v
            else:
    ...
    with col_radar:
        ...
        for k in VERTEX_KEYS:
            ...
            legend_parts.append(
                ...
                f'<span style="font-size:11px;color:#6e6e73;width:130px;flex-shrink:0;">'
                f'{VERTICES[k]["label"]}</span>'
                f'<div style="flex:1;height:4px;background:#2c2c2e;border-radius:2px;overflow:hidden;">'
                f'<div style="width:{pct:.0f}%;height:100%;background:#f5f5f7;'
                f'border-radius:2px;transition:width 0.3s;"></div></div>'
                f'<span style="font-size:11px;color:#aeaeb2;width:36px;text-align:right;">'
                f'{pct:.0f}%</span></div>'
            )
        st.markdown(f"""
        <div style="background:#1c1c1e;border:1px solid #2c2c2e;
...
def step_estilo():
    ...
    with col1:
        vida_social = st.select_slider(
            ...
            options=opciones,
            value=next((k for k, v in peso_map.items()
                       if v == finder.get("w_vida_social", 7)), "Importante"),
        ...
        restaurantes = st.select_slider(
            ...
            options=opciones,
            value=next((k for k, v in peso_map.items()
                       if v == finder.get("w_restaurantes", 3)), "Algo importante"),
    ...
    with col2:
        seguridad = st.select_slider(
            ...
            options=opciones,
            value=next((k for k, v in peso_map.items()
                       if v == finder.get("w_seguridad", 10)), "Muy importante"),
            help="Sentirte seguro en tu día a día"
        )
        remoto = st.toggle(
            "Trabajo en remoto",
            value=finder.get("remoto", False),
...
def _explain_city(row, prefs):
    ...
    blocks  = CULTURAL_BLOCKS.get(country, {})
    if aff >= 100:
        pros.append("Alta afinidad cultural (Occidental · Mediterráneo)")
    elif aff >= 55:
...
def step_resultado(df_full):
    ...
    for i, row in top5.iterrows():
        ...
        country    = row.get("country", "")
        blocks     = CULTURAL_BLOCKS.get(country, {})
        cult_badge = (f'{blocks.get("general","")} · {blocks.get("specific","")}') if blocks else ""
            ...
            justify-content:space-between;margin-bottom:16px;">
                <div style="flex:1;">
                    ...
                    gap:10px;margin-bottom:8px;flex-wrap:wrap;">
                        <span style="font-size:13px;color:{rank_colors[i]};font-weight:700;">
                            #{i+1}
                        </span>
                        <span style="font-size:22px;font-weight:600;color:#f5f5f7;
                        letter-spacing:-0.02em;">{row['city_name']}</span>
                    ...
                    <div style="display:flex;gap:8px;flex-wrap:wrap;">
                        <span style="font-size:10px;color:#6e6e73;background:#2c2c2e;
                        border-radius:6px;padding:3px 8px;">{cult_badge}</span>
                        <span style="font-size:10px;color:#6e6e73;background:#2c2c2e;
                        border-radius:6px;padding:3px 8px;">Estabilidad {stab}/100</span>
                    </div>
                </div>
                <div style="text-align:right;min-width:90px;margin-left:20px;">
                    ...
                    <div style="font-size:9px;color:#6e6e73;letter-spacing:0.08em;
                    text-transform:uppercase;margin-top:2px;">compatibilidad</div>
    ...
    fig.update_layout(
        ...
        paper_bgcolor="rgba(0,0,0,0)",
        showlegend=True,
        legend=dict(font=dict(color="#aeaeb2", size=11), bgcolor="rgba(0,0,0,0)"),
        height=420,
    ...
    with col2:
        ...
        st.button("Comparar top 3 en City Comparison →",
                  type="primary", use_container_width=True,
